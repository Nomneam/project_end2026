...
async function submitProfileUpdate(formData, triggerBtn) {
  ...
  try {
    ...
    console.error(err);
    if (hasSwal()) Swal.fire("ผิดพลาด", err.message || "ไม่สามารถอัปเดตข้อมูลได้", "error");
    else alert(err.message || "อัปเดตไม่สำเร็จ");
  } finally {
...
async function submitChangePassword(form) {
  ...
  try {
    ...
    const data = await res.json();
    if (!res.ok || !data.success) throw new Error(data.error || "Change password failed");

    await showSuccess("เปลี่ยนรหัสผ่านเรียบร้อยแล้ว");
    location.reload();
  } catch (err) {
    console.error(err);
    if (hasSwal()) Swal.fire("ผิดพลาด", err.message || "เปลี่ยนรหัสผ่านไม่สำเร็จ", "error");
    else alert(err.message || "เปลี่ยนรหัสผ่านไม่สำเร็จ");
  }
}
...
function bindEditProfile() {
  ...
  editBtns.forEach((btn) => {
    btn.addEventListener("click", (e) => {
      ...
      if (label.includes("ชื่อ") && !label.includes("นามสกุล")) fieldName = "emp_fname";
      if (label.includes("นามสกุล")) fieldName = "emp_lname";
      ...
      valEl.innerHTML = `
        <input type="${inputType}" class="form-control form-control-sm mb-1" value="${oldValue}">
      ...
      saveBtn.onclick = async () => {
        ...
        if (!newValue) return showError("กรุณากรอกข้อมูล");
        if (fieldName === "emp_phone" && !isValidPhone(newValue)) return showError("กรุณากรอกเบอร์โทร 10 หลัก");
        if (fieldName === "emp_email" && !isValidEmail(newValue)) return showError("รูปแบบอีเมลไม่ถูกต้อง");
